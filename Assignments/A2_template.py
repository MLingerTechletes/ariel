...
os.environ.setdefault("MUJOCO_EGL_DEVICE_ID", "-1") # any GPU / fallback
...
# All pre-made fitness functions
from ariel.simulation.tasks.gate_learning import xy_displacement_ff, x_speed_ff, y_speed_ff
from ariel.simulation.tasks.targeted_locomotion import distance_to_target_ff
from ariel.simulation.tasks.turning_in_place import turning_in_place_ff
...
def show_qpos_history(history:list):
    ...
    plt.savefig(os.path.join("Assignments/", f"Assign2_plot_{timestamp}.png"), dpi=150)
    plt.show()
...
def main():
    ...
    start_xy = HISTORY[0][:2]
    end_xy = HISTORY[-1][:2]
    fitness = xy_displacement_ff(start_xy, end_xy)
